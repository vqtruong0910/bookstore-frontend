...
function AuthorManagement() {
    ...
    const { data: authors, isLoading, isError } = useQuery(['authors', page], async () => {
    ...
    const { mutateAsync: addAuthor } = useMutation(async (data) => {
    ...
    const { mutateAsync: updateAuthor } = useMutation(async (data) => {
    ...
    const { mutateAsync: deleteAuthor } = useMutation(async (idProduct) => {
    ...
    const nextPage = useCallback(() => {
        setPage(currentPage => currentPage >= authors.SoLuongTrang ? authors.SoLuongTrang : ++currentPage);
    }, [page, authors]);
    ...
    const changePage = useCallback((e) => {
        ...
        if (value > 0 && value <= authors.SoLuongTrang)
            setPage(value);
    }, [authors])
        ...
        <>
            ...
            <div className="py-2 space-y-1">
                <div className="hidden md:flex">
                    <form onSubmit={handleSubmit(handleAddAuthor)} className="flex space-x-3">
                        <input {...register("TenTacGia", { required: true })} placeholder="Tên tác giả..." type="text" className={clsx(errors["TenTacGia"] && "border-red-600", "focus:outline-none rounded-sm border")} />
                        <input {...register("DiaChi", { required: true })} placeholder="Địa chỉ..." type="text" className={clsx(errors["DiaChi"] && "border-red-600", "focus:outline-none rounded-sm border")} />
                ...
                <div className="flex justify-end space-x-2 py-1">
                    ...
                    <div><input type="number" className={clsx(style["none-spin"], 'px-2 border rounded-sm cursor-pointer w-14 text-center')} value={page} onChange={(e) => changePage(e)} /> &#47; {authors?.SoLuongTrang}</div>
            ...
            <table className="border-collapse border rounded-sm w-full bg-white">
                <thead>
                    <tr className="border bg-slate-800 text-slate-200">
                        <th className="p-2 text-left">Tên tác giả</th>
                ...
                <tbody>
                    {authors && authors.DanhSach.map(item => (
                        <tr key={id + item.IDTacGia} className="odd:bg-slate-100 border">
                            ...
                            <td className="p-2 text-indigo-500 font-semibold w-16 align-middle"><span className="w-full"><BsPencilSquare onClick={() => showUpdateAuthor(item)} size={20} className='cursor-pointer mx-auto' /></span></td>
                            <td className="p-2 text-red-500 font-semibold w-16 align-middle"><span className="w-full"><AiFillDelete onClick={() => handleDeleteAuthor(item.IDTacGia)} size={20} className='cursor-pointer mx-auto' /></span></td>
                        </tr>
                    ))}
                </tbody>
            </table>
            ...
            <div className={clsx(formUpdate ? "z-30" : "-z-30", "fixed flex justify-center items-center inset-0 bg-slate-900 bg-opacity-20")}>
                <div className={clsx(!formUpdate && "scale-0", style["hide-scrollbar"], "flex w-full flex-col bg-white rounded-sm overflow-y-scroll sm:w-1/2 max-h-screen transition-all duration-200")}>
                    <form onSubmit={handleSubmitUpdate(handleUpdateAuthor)} className="w-full flex flex-col py-4 px-4 sm:p-6">
                        ...
                        <div className="flex flex-col w-full justify-center h-full sm:px-10">
                            <div className="flex w-full py-2">
                                <div className="w-1/3 lg:w-4/12 items-center flex">
                                    <span className="flex text-sm lg:text-base font-semibold text-slate-800">Tên Tác Giả</span>
                                </div>

                                <div className="w-2/3 lg:w-8/12 flex">
                                    <input {...registerUpdate("TacGia")} className="w-full border rounded-sm px-2 py-1 lg:py-2 focus:outline-none focus:ring-sky-200 focus:ring-1 placeholder:text-slate-400 placeholder:text-sm lg:placeholder:text-base" type="text" placeholder="Tên danh mục" />
                                </div>
                            ...
                            <div className="flex w-full py-2">
                                ...
                                <div className="w-2/3 lg:w-8/12 flex">
                                    <input {...registerUpdate("DiaChiUpdate")} className="w-full border rounded-sm px-2 py-1 lg:py-2 focus:outline-none focus:ring-sky-200 focus:ring-1 placeholder:text-slate-400 placeholder:text-sm lg:placeholder:text-base" type="text" placeholder="Địa chỉ" />
                                </div>
