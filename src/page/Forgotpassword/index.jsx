...
function Forgotpassword() {
    ...
    const onSubmit = useCallback(async (data) => {
        try {
            ...
            if (!result.error) {
                window.alert("Chúng tôi đã gửi 1 email xác thực đến tài khoản email của bạn\n Lưu ý bạn không được gửi đường link này cho bất cứ ai!!!");
            }
        } catch (error) {
            if (error?.response?.data?.error) {
                setError("Email", { type: "custom", message: "Tài khoản email không tồn tại" }, { shouldFocus: true })
            } else {
