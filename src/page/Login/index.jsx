...
function Login() {
  ...
  const sendEmailAgain = useCallback(async (Email) => {
    try {
      ...
      window.alert(
        "Chúng tôi vừa gửi 1 link xác nhận email đến tài khoản email của bạn!"
      );
    } catch (error) {
  ...
  const onSubmit = useCallback(async (data) => {
    try {
      ...
      if (result === "0") {
        setError(
          "Email",
          { type: "custom", message: "Tài khoản không tồn tại" },
          { shouldFocus: true }
        );
      } else if (result === "1") {
        setError(
          "MatKhau",
          { type: "custom", message: "Mật khẩu sai xin nhập lại" },
          { shouldFocus: true }
        );
      } else if (result === "2") {
        setError(
        ...
        window.alert(
          "Tài khoản của bạn đã bị khóa/n Vui lòng liên hệ Admin để hiểu rõ thêm."
        );
      } else {
  ...
  return (
    <div className="flex flex-col rounded-lg bg-white shadow-xl border">
      <h3 className="text-center p-5 text-lg font-normal">
        Đăng nhập BOOKSTORE
      </h3>
      ...
      <span className="text-xs p-5 block text-center text-slate-600 select-none">
        Hoặc Đăng Nhập Bằng
      </span>
      <div className="flex justify-center items-center space-x-2 mb-5">
        <div className="cursor-pointer" onClick={loginFaceBook}>
          <MdFacebook size={40} color="blue" />
        </div>
        <div className="cursor-pointer" onClick={loginGoogle}>
          <FcGoogle size={40} />
        </div>
      </div>
